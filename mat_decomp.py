...
class _SupportsShape(Protocol):
    def shape(self) -> Tuple[int, ...]:  # noqa: D401 – simple protocol
        ...


from typing import Union

def _tensor_to_numpy(t: Union[Tensor, UniTensor, np.ndarray]) -> np.ndarray:

    """Convert cytnx :class:`Tensor`/`UniTensor` *or* ndarray to *ndarray*.

    If a **UniTensor** is passed, we take its `.get_block()` (the dense
    data tensor).  For *Tensor* we use `.numpy()` (copy – cytnx does not
    share memory with NumPy).
    """
    if isinstance(t, np.ndarray):
        return t
    if isinstance(t, UniTensor):
        return t.get_block().numpy()
    if isinstance(t, Tensor):
        return t.numpy()
    raise TypeError(f"Unsupported type {type(t)} for tensor conversion")
